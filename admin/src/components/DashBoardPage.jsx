...
import { User, UserCheck, Calendar, IndianRupee, CheckCircle, XCircle, Search } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function loadPatientCount() {
      ...
      try {
        ...
        const body = await res.json().catch(() => ({}));
        // Added body?.userCount to match your backend JSON response key
        const count = Number(
          body?.userCount ?? body?.count ?? body?.totalUsers ?? body?.data ?? 0
        );
        ...
        <div className="mb-6">
          ...
          <div className={s.searchContainer}>
            <div className={s.searchInputContainer}>
              {/* FIX 1: Moved Search Icon here so it renders on the left side of the input field */}
            ...
            {/* Added standard green pill styling directly to clear button class to mirror tutorial layout precisely */}
            <button
              onClick={() => {
                setQuery("");
                setShowAll(false);
              }}
              className="bg-[#00b562] text-white px-5 py-1.5 rounded-full text-sm font-medium hover:bg-green-600 transition-all cursor-pointer"
            >
              Clear
            </button>
          </div>
        ...
        <div className={s.tableContainer}>
          ...
          <div className={s.tableWrapper}>
            <table className={s.table}>
              <thead>
                <tr>
                  {/* FIX 2: Normalized headers to uppercase strings to mirror the reference image text structure */}
