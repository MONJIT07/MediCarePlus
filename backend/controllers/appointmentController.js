...
const stripe = STRIPE_KEY ? new Stripe(STRIPE_KEY, { apiVersion: "2026-06-24.dahlia" }) : null;
...
// this function will get the user from clerk
...
export const createAppointment = async (req, res) => {
  try {
    ...
    if (numericFee == null || numericFee < 0) return res.status(400).json({ success: false, message: "Fees must be a Valid number" });
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    const updated = await Appointment.findByIdAndUpdate(id, update, { new: true }).populate({ path: "doctorId", select: "name imageUrl" }).lean();
