...
// Map imports to match variables used throughout the file
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (!session) return res.status(404).json({ success: false, message: "Stripe session not found" });
...
export const getServiceAppointments = async (req, res) => {
  try {
    ...
    const appointments = await ServiceAppointment.find(filter).populate("serviceId", "name imageUrl image imageSmall").sort({ createdAt: -1 }).skip(skip).limit(limit).lean();
...
export const cancelServiceAppointment = async (req, res) => {
  try {
    const { id } = req.params;
    // Removed .lean() since we mutate properties and perform an instance-level .save() later
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    // Renamed local result container to avoid duplicate declaration of imported "services"
    const statsResult = await Service.aggregate([
      {
    ...
    return res.json({ success: true, services: statsResult, total: statsResult.length });
  } catch (err) {
...
export const getAppointmentForPatient = async (req, res) => {
  try {
    ...
    console.error("getAppointmentForPatient error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
